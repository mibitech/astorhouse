...
- A chave `anon`/publishable do Supabase é **pública por design** (protegida por RLS) — não é finding; confirme apenas que **nenhum `service_role`** aparece em `src/`
...
- Verifique se políticas usam `auth.uid()` / `is_admin()` e não dados client-side
...
### 5. Dados compartilhados — tabela `Leads`

- Mudanças na tabela `"Leads"` (compartilhada com a assistente Léssie) — confirme que as policies de insert anon e a PK `id_conversa` (dedup) não foram quebradas

### 6. OWASP Top 10 — Verificações Rápidas
